...
const gameState = {type: 'server_gstate', endGame: 0, x: 0, title: '', content: '', keysActive: true, statRight: 0, statWrong: 0,
 currGameBoard: Array(6).fill(null).map(() => Array(5).fill(null).map(() => ({letter: '', colour: 'not_seen'}))), timer: gameLength,
 currColours: Object.fromEntries(Array.from({length: 26}, (_, i) => [String.fromCharCode(i + 65), 0])), target: Game.getTarget()};
let timerID;
...
function resetGameState() {
    ...
    gameState.currGameBoard = Array(6).fill(null).map(() => Array(5).fill(null).map(() => ({letter: '', colour: 'not_seen'})));
    gameState.currColours = Object.fromEntries(Array.from({length: 26}, (_, i) => [String.fromCharCode(i + 65), 0]));
...
function startGameTimer() {
	...
	timerID = setInterval(() => {
		gameState.timer -= 1;
		Object.values(playing).forEach(webS => {webS.send(JSON.stringify({type: 'timer', timer: gameState.timer}));})
		if(gameState.timer <= 0) {
			clearInterval(timerID);
			serverUpdateThings([], 'times up');
			Object.values(playing).forEach(webS => {webS.send(JSON.stringify(gameState));})
			Game.state = 'lost';
		}
	}, 1000);
}
...
function serverUpdateThings(score=[], state='') {
	score.forEach((scorChar, i) => {
		...
		gameState.currColours[char] = gameState.currColours[char] < charScore ? charScore : gameState.currColours[char];
	...
	if (state === 'won' || state === 'lost' || state === 'times up') {
		...
		gameState.endGame = 1;
		if (state === 'won') {gameState.statRight += 1} else {gameState.statWrong += 1};
	}
  }
...
app.get('/api/username/', function (req, res) { // first connect
	...
	if (sessionID && sessionID in sessions) {
		...
		do {username=Game.makeUsername();} while (Array.from(wsPMap.values()).includes(username));
		sessionID = id;
...
app.put('/api/username/:username/newgame', function (req, res) { // new game
	...
	if (ws !== null) {playing[uname] = ws;} else {console.error('Player not found?'); res.status(404);}
	res.end();
